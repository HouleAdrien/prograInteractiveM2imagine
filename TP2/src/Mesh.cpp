...
void Mesh::compute_skinning_weights( Skeleton & skeleton ) {
	...
	for(int i = 0; i < V.size(); i++) {
	    ...
	    for(int j = 0; j < skeleton.bones.size(); j++) {
	    	Articulation & a0 = skeleton.articulations[skeleton.bones[j].joints[0]];
	        Articulation & a1 = skeleton.articulations[skeleton.bones[j].joints[1]];

	        Vec3 a0pos = a0.p;
	        Vec3 a1pos = a1.p;
	        Vec3 vertexPoint = V[i].p; 

	        // calcul de la projection
	        Vec3 v = a1pos - a0pos;
	        Vec3 w = vertexPoint - a0pos;

	        double c1 = Vec3::dot(w,v);

	        //plus proche de l'articulation 0 que de l'os lui même
	        if (c1 <= 0) {
	            float distance = (vertexPoint - a0pos).length();
	            V[i].w[j] =  pow((1.0 /distance), n);
	            totalWeight += V[i].w[j];
	            continue;
	        }

	        double c2 = Vec3::dot(v,v);


	        //plus proche de l'articulation 1 que de l'os lui même
	        if (c1 >= c2) {
	            float distance = (vertexPoint - a1pos).length();
	            V[i].w[j] =  pow((1.0 /distance), n);
	            totalWeight += V[i].w[j];
	            continue;
	        }
	        ...
	        //cas général
	        float b = c1 / c2;
	        Vec3 projectedPoint = a0pos + b * v;

	        float distance = (vertexPoint - projectedPoint).length();
	        V[i].w[j] =  pow((1.0 /distance), n);
	        totalWeight += V[i].w[j];
	    }
...
void Mesh::drawTransformedMesh( SkeletonTransformation & transfo ) const {
    ...
    for( unsigned int i = 0 ; i < V.size() ; ++i ) {
        ...
        for(int j=0; j< transfo.bone_transformations.size() ;j++){
        	float we = V[i].w[j];
        	Vec3 t  =transfo.bone_transformations[j].world_space_translation;
        	Mat3 r = transfo.bone_transformations[j].world_space_rotation;
        	new_positions[ i ] += we * (r*p + t);
            new_normals[ i ] += we * (Mat3::inverse(r).getTranspose() * n);
        }
